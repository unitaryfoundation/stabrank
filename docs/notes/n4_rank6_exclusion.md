...
6 <= chi(N^4) <= 7 (`bounds/N-m4-lower-6.json`, the rank-5 exclusion of
`docs/notes/qutrit_m4_rank5_exclusion.md`; `bounds/N-m4-upper-7.json`, the
Lean witness). A clean run of the batches of `research/n4_rank6/` settles
chi(N^4) = 7. The design was chosen and measured in
`docs/notes/n4_rank6_design.md`; this note is the argument as implemented,
written so that it can be checked from the files alone.
...
(design note, section 3.1). Open before the bound: the pod run itself, its
aggregate, the `--no-native` replays, and the B6 pod rate (the partition
takes the Python factor on the filter's bookkeeping; the first B6 batches
decide between 50 and 120 pod CPU-hours). Recorded rather than closed: the
