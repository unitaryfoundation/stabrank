...
## 2026-09-24: the sector route at every prime (sectors_p.py)

Script: `research/constructions/sectors_p.py`, the generalization of
`sectors.py` to p in {2, 3, 5} and to groups generated by one or two
commuting Pauli strings, with `--control` (dense checks of the Clifford
reduction at p = 2, 3, 5; the T5 m=2 identity; the T3 m=3 and N m=3 facts
of section 3; the sector-weight formula against explicit projections).
Everything ran as one process at nice 19 under the 600 s cap of
`research/t5_rank5/run.py`, 30 runs, about 73 minutes of wall time; the
annealing runs used 4 or 8 chains, so the CPU time is between 1.2 and about
3.5 CPU-hours depending on how the chains were scheduled. The literature
update of the same day is section 8 of `literature_sweep_2026_09.md`
(nothing new). No bound file was written: no construction reached a rank
below the board.

### Record-capable open cells

Computed from `bounds/*.json` (best bound per cell; lower bounds carried
forward by projection monotonicity): the ranks r with log_p(r)/m below the
published exponent, or below the board's best exponent 0.5 for T5, and r
between the lower bound and the current upper bound.

| cell | board | record ranks (exponent) |
|---|---|---|
| S m=5 | 5..8 | 5 (0.2930) |
| S m=6 | 5..8 | 5..7 (0.2442..0.2952) |
| S m=7 | ..16 | 5..11 (0.2093..0.3118) |
| S m=8 | ..16 | 5..15 (0.1831..0.3081) |
| N m=4 | 6..7 | 6 (0.4077) |
| N m=5 | ..12 | 6..10 (0.3262..0.4192) |
| N m=6 | ..16 | 6..15 (0.2718..0.4108) |
| H3 m=4 | 6..8 | 6 (0.4077) |
| H3 m=5 | ..12 | 6..10 |
| H3 m=6 | ..16 | 6..15 |
| T3 m=4 | 8..9 | 8 (0.4732) |
...
| qubit_H m=9 | ..18 | 6..11 |
| qubit_H m=10 | 6..18 | 6..15 |
| qubit_T m=7 | no cell (6 by projection, 9 by product) | 6 |
| qubit_T m=8 | ..9 | 6..8 |
| qubit_T m=9, 10 | .., ..18 | 6..11, 6..15 |
| T5 m=3 | 5..15 | 5..11 (0.3333..0.4966) |
| T5 m=4 | 5..25 | 5..24 (0.2500..0.4937) |

Settled this week and no longer targets: qubit_H m=5, 6 (= 6), qubit_T
m=5, 6 (= 6), T5 m=2 (= 5), T3 m=3 (= 8); N m=4 and H3 m=4 lower bounds 6.

### The method

For an abelian group G of Pauli strings with p^k elements and full support
on the m copies, |M>^m = sum_s Pi_s |M>^m over the p^k joint eigensectors,
each inside the code {P_i = w^{s_i}}, and a Clifford with C P_i C^-1 = Z_i
(built from the symplectic rules for F, S, the multiplier |x> -> |ux>, SUM,
and SWAP, applied to vectors as index and phase operations) turns the
sector into |s> (x) u_s with u_s an (m-k)-qudit state. chi(|M>^m) <= sum_s
chi(u_s), and chi(u_s) is decided exactly in the (m-k)-qudit dictionary
when it fits (five qubits, 2,423,520 states, 240 s to enumerate and cached
on disk; three qutrits; two ququints): 1 by the stabilizer test, 2 by
`rank2_search`, 3 by `rank3_search` with `--rank3`. Without a dictionary
the script reports 0, 1, or ">= 2" and, with `--anneal R`, runs the
board's annealer on u_s at rank R (8000 iterations per temperature,
cooling 0.995, 4 or 8 chains), which is a search, not an exclusion.

For k = 2 the pairs are screened first by the number of nonvanishing
sectors, which is cheap: the weight of sector s is the Fourier transform
over G of <psi|g|psi> = prod_i <M|g_i|M>. Every nonvanishing sector costs
at least one term, so pairs with at least as many nonvanishing sectors as
the cell's current bound are dropped before any rank test. The survivors
are deduped by a canonical form of the group under the local stabilizer of
|M> on each copy, the copy permutations, and the change of generators
(GL_2(F_p)).

Two identifications worth recording. For the qubit H-type state,
T_perp = -Y T, so QPG's cat_m = (T^m + T_perp^m)/sqrt 2 and its partner
cat_m^- are exactly the two Y^{(x) m} eigensectors of |T>^m; the Y^6 control
below reproduces chi(cat_6) = 3. For the face state F no Pauli maps F to
F_perp (the Pauli group flips an even number of Bloch signs and F_perp is
the antipode), so QPG's cat_6(F) is not a Pauli sector of |F>^6, and the
Y^6 sectors of |F>^6 have rank >= 3 exactly with rank 3 not found by the
annealer (residuals 0.19 and 0.32).

### Results, one line per run

Exact means decided in the dictionary; annealed means a rank-R search
that did not reach residual 1e-9 (the best residual is given).

| cell | group | classes | outcome |
|---|---|---|---|
| T5 m=3 | k=1, all 120 full-support classes | 120 | exact: every sector of every string has rank >= 3, so the sector total is >= 15 = the product for every string; with `--rank3` the first 55 classes (all the strings with at least one Z-type site scanned before the cap) have every sector of rank > 3, total >= 20 |
| T5 m=3 | k=2, all commuting independent pairs | 331,200 pairs | nonvanishing-sector histogram {20: 2160, 23: 2080, 24: 4272, 25: 322,688}: no pair has fewer than 15 nonvanishing sectors, so no two-Pauli sector decomposition gets below 20 |
| T5 m=4 | k=1, Z^4 | 1 | the sector is the sum of the five (Z Z sector) (x) (Z Z sector) products, all stabilizer after the reduction, linearly independent, no four of them spanning the sector; rank <= 5 per sector (the product 25). Annealed at rank 4: residual 0.65; the rank-5 control on the same 125-dimensional state also fails (residuals 0.48 and 0.62), so the p = 5 annealer says nothing here |
| T5 m=4 | k=2, all pairs | 21,896,160 pairs, 624 classes with 20 nonvanishing sectors | exact: in every one of the 624 classes each of the 20 nonvanishing sectors (two-ququint states) has rank >= 3, total >= 60 |
| T3 m=4 | k=2 | 47,740 pairs | every pair has all 9 sectors nonvanishing, total >= 9 = the board |
| T3 m=5 | k=2 | 2204 classes | exact rank-2 tests on the three-qutrit sectors: smallest total 24 (Z^5 with Z Z X X X: three sectors of rank 2, six of rank >= 3); 2200 classes at >= 27, one at >= 26, three at >= 24. The two-Pauli route is at least 24 against the board's 18 |
| N m=4 | k=2 | 4 classes with 6 nonvanishing sectors (X-type, 20 pairs) | exact: totals >= 16, 18, 18, 18 |
| H3 m=4 | k=2 | 6820 pairs | all 9 sectors nonvanishing for every pair, >= 9 |
| N m=5 | k=2 | 223 classes | exact: 5 classes at >= 18 (six nonvanishing sectors of rank >= 3), 1 at >= 25, 217 at >= 27; board 12 |
| H3 m=5 | k=2 | 602 classes | exact: every sector rank >= 3, >= 27 |
| S m=5 | k=2 | 1 class with 6 nonvanishing sectors ((X^2 Z^2)^5 with (X Z)^4 X^2 Z^2) | exact: all six sectors rank >= 3, >= 18 |
| S m=6 | k=2 | 1 class with 6 nonvanishing sectors | four-qutrit sectors, none a stabilizer state, >= 12 |
| S m=5 | k=1 (one class, (X^2 Z^2)^5) | 1 | three four-qutrit sectors, weights 0.34, 0.34, 0.31; annealed at rank 2: residuals 0.82, 0.82, 0.84 |
| S m=6 | k=1 | 1 | three five-qutrit sectors; annealed at rank 2: 0.89, 0.91, 0.91 |
| N m=5 | k=1 | 6 | one four-qutrit sector per class annealed at rank 3: residuals 0.59 to 0.72 |
| H3 m=5 | k=1 | 6 | same: residuals 0.32 to 0.55 |
| qubit_H m=6 | k=1, Y^6 (control) | 1 | exact rank >= 3 in the five-qubit dictionary, annealed rank 3 found in 7 s per sector: chi(cat_6) = 3 |
| qubit_H m=7 | k=1, all 8 classes | 8 | six-qubit sectors annealed at rank 3: residuals 0.30 to 0.54 (Y^7, the cat_7 pair: 0.54, 0.54); Y^7 at rank 4: 0.42, 0.42 |
| qubit_H m=7 | k=2 | 92 classes (all pairs have exactly 4 nonvanishing sectors) | exact in the five-qubit dictionary: every sector of every class has rank >= 3, total >= 12 against the board's 9 |
| qubit_T m=7 | k=1 (one class, Y^7) | 1 | annealed at rank 3: 0.40, 0.53 |
| qubit_T m=7 | k=2 | 9 classes | exact: every sector rank >= 3, >= 12 |
| qubit_H m=8 | k=1, Y^8 (cat_8) and X^8 | 2 | seven-qubit sectors annealed at rank 4: 0.45 (Y^8, one sector), 0.36 and 0.41 (X^8) |
| qubit_T m=8 | k=1, Y^8 | 1 | annealed at rank 4: 0.50 |

Not run: k = 1 at N, H3 m=6 and S m=7, 8 (the sectors are five- to
seven-qutrit states with no dictionary, and the k = 1 anneals one copy
down all failed); k = 3 (2^3 or 3^3 sectors, each costing at least one
term, exceed every record rank on the board except at S m=7, 8 and T3 m=6,
where the sectors would be five- and three-qutrit states; the T3 m=6 case
is the T3 m=4 statement three times, >= 27); non-Pauli Cliffords (their
eigenspaces are not stabilizer codes, as section 3 says).

### Sharpest facts of the session

1. T5 m=3: every Pauli eigensector of |T5>^3, for every full-support
   string, has rank >= 3 (exact), so the sector route reproduces the
   product 15 and nothing below it; the Z-type sectors have rank exactly
   4 or 5 (rank 3 excluded exactly, rank 5 by the five sub-sector
   products), so Z-type sector totals are 20 or 25. The two-copy identity
   chi(|T5>^2) = 5 has no one-Pauli or two-Pauli analogue at m = 3 or
   m = 4 (two-Pauli totals >= 20 and >= 60).
2. Two-Pauli sectors never beat the board anywhere: the totals are
   >= 24 (T3 m=5), >= 16 (N m=4), >= 18 (N m=5, S m=5), >= 27 (H3 m=5),
   >= 12 (qubit_H m=7, qubit_T m=7, S m=6), >= 60 (T5 m=4), all exact
   except the S m=6 sectors, which are only known not to be stabilizer
   states. Vanishing sectors occur only for X-type groups on N and S and
   for mixed groups on T5, and never in enough number.
3. The board's annealer, with the settings that recover chi(cat_6) = 3 in
   7 s, finds no rank-3 sector for |H>^7 or |F>^7 under any Pauli string,
   no rank-4 sector for |H>^8 or |F>^8 under Y^m or X^m, no rank-2 sector
   of |S>^5 or |S>^6, and no rank-3 sector of |N>^5 or |H3>^5. Heuristic.
   The p = 5 annealer fails its own rank-5 control on the 125-dimensional
   T5 sector, so the T5 m=4 rank-4 question (which would give chi(T5^4)
