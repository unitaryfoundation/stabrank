...
def run_qubit_real_example():
    """
    Runs an example of finding a stabilizer rank decomposition for the Qubit Hadamard Eigenstate.
    Using restricted 'Real-Preserving' moves (Even Ys).
    """
    print("\n--- Qubit Real-Amplitude Decomposition Example ---")
    ...
    # derived so that 1.0 * (rate)^100 approx 1/4000
    cooling_rate = 0.995 
    ...
    n_orig = 6   # Number of qubits
    p_prime = 2  # Qubits
    
    # Target: Real-valued Hadamard state
    target_func = qubit_hadamard_eigenstate(n_orig)
    
    # Start with k=10
    k_start = 6
    
    # Dummy subspace params for random generation (Full space)
    k_dim_poly = n_orig
    W_basis = np.eye(n_orig, dtype=int)
    x0_trans = np.zeros(n_orig, dtype=int)

    # Initial Basis: n copies of the uniform superposition (|+>^n) 
    # Or random stabilizer states. Let's use Random Pauli Stabilizers.
    print(f"Generating initial random basis of {k_start} functions...")
    initial_basis = []
    
    # Generator for new random functions (Standard stabilizer states)
    def generate_random_func():
        # Generate generic quadratic form
        # Note: generate_random_poly_coeffs handles p=2 correctly
        coeffs = generate_random_poly_coeffs(k_dim_poly, p_prime, [0.0])
        # Evaluate to get the vector
        func = evaluate_coeffs_on_subspace(coeffs, n_orig, p_prime, x0_trans, W_basis)
        return func

    for _ in range(k_start):
        func = generate_random_func()
        # func = np.ones(2**n_orig)
    ...
    while current_k > 0:
        ...
        # Run SA
        # We run multiple passes? Let's just do one solid pass per k level for this test.
        runs = 1
        ...
        if found_solution:
            ...
            filename = f"solution_qubit_real_k{current_k}_n{n_orig}_{timestamp}.npz"
            ...
            while current_k > 0:
                ...
                if err < ERROR_THRESHOLD:
                    ...
                    # We need the linear coefficients for saving
                    ...
                    np.savez_compressed(filename, 
                                        ...
                                        **{f'basis_func_{i}': f for i, f in enumerate(current_best_funcs)})
                    print(f"Saved to {filename}")
                    # Continue the while loop to see if we can prune AGAIN!
                else:
                    # The pruned basis does NOT satisfy the threshold.
                    # We need to run SA for this new current_k.
                    break
        else:
