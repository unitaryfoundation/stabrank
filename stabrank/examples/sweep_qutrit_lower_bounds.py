"""Sweep all qutrit magic code states for a given (m, k) and compute lower bounds.

Usage:
    uv run python -m stabrank.examples.sweep_qutrit_lower_bounds --m 5
    uv run python -m stabrank.examples.sweep_qutrit_lower_bounds --m 5 --k 1
    uv run python -m stabrank.examples.sweep_qutrit_lower_bounds --m 6 --k 1 --k 2

Enumerates all systematic [m, k] ternary codes G = [I_k | A] with A in F_3^{k x (m-k)},
...
from stabrank.target_functions import qutrit_magic_code_state_compressed
...
def sweep_lower_bounds(m: int, k_values: list[int] | None = None) -> None:
    ...
    csv_filename = f"sweep_lower_bounds_m{m}_{timestamp}.csv"
    ...
    with open(csv_filename, "w", newline="") as f:
        ...
        writer.writerow([
            ...
            # Per-dimension columns: f_max_k0, f_max_k1, ...
            *[f"f_max_k{kk}" for kk in range(m)],
        ])

    print(f"=== Sweep Lower Bounds for m={m} ===")
    ...
    for k in k_values:
        ...
        for A_idx, A_flat in enumerate(
            ...
            psi = qutrit_magic_code_state_compressed(G)
            ...
            # Which stabilizer dimension gives the best overlap?
            ...
            # Pad f_per_k to m columns (max possible n_eff = m-1)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Sweep qutrit code states and compute stabilizer rank lower bounds."
    )
    parser.add_argument("--m", type=int, required=True, help="Number of qutrits m.")
    parser.add_argument(
