...
* **Layer 4** — Main case verification (`qubit_t_m4_decomposition`): `fin_cases` over the
  16 basis points, one controlled `simp only` that pins the normal form, then
  `linear_combination` over `I * I = -1` and `√2 * √2 = 2`, nine coefficient patterns
  dispatched by `first`.
* **Layer 5** — Assembly (`stabilizer_rank_le_three` and `gamma_T_bound`), giving the exact
  3-term stabilizer decomposition and the numerical bound on the exponent.
-/
...
lemma cos_sub_sin_pi_div_12 :
  ...
  have h_div : (Real.sqrt 6 + Real.sqrt 2) / 4 - (Real.sqrt 6 - Real.sqrt 2) / 4
...
/-- `1/(2√2) = √2/4`, so that every σ-amplitude is a polynomial in `↑√2`
    and no division by an atom survives into the case analysis. -/
lemma inv_two_sqrt2 : ((1 / (2 * Real.sqrt 2) : ℝ) : ℂ) = (Real.sqrt 2 : ℂ) / 4 := by
  have h : (1 / (2 * Real.sqrt 2) : ℝ) = Real.sqrt 2 / 4 := by
    have hs := sqrt2_sq
    have hn := sqrt2_ne_zero
    field_simp
    linarith
  rw [h]
  push_cast
...
/-- `c₁ = (√2/6)((√3 + 1) + i(√3 - 1))`, the closed form in the atoms
    `√2, √3, i`; `√6` is eliminated through `sqrt6_eq`. -/
lemma c1_eq : c1 = (Real.sqrt 2 : ℂ) / 6 *
    (((Real.sqrt 3 : ℂ) + 1) + Complex.I * ((Real.sqrt 3 : ℂ) - 1)) := by
  rw [c1_expand, cos_pi_div_12, sin_pi_div_12, sqrt6_eq]
  push_cast
  ring

/-- `c₃ = (√2/6)((√3 + 1) - i(√3 - 1))`. -/
lemma c3_eq : c3 = (Real.sqrt 2 : ℂ) / 6 *
    (((Real.sqrt 3 : ℂ) + 1) - Complex.I * ((Real.sqrt 3 : ℂ) - 1)) := by
  rw [c3_expand, cos_pi_div_12, sin_pi_div_12, sqrt6_eq]
  push_cast
  ring

-- Same shape as the other Pointwise files: `fin_cases`, one controlled
-- `simp only` that pins the normal form (powers of `I` reduced to `I` and
-- `-1`, the coefficients in closed form), then `linear_combination` over
-- `I * I = -1` and `√2 * √2 = 2`. `√3 * √3 = 3` is never needed: `√3` enters
-- every residual at most linearly. The coefficient pairs were computed by
-- reducing each residual against the ideal generated by those two relations.
set_option linter.flexible false in
set_option linter.style.longLine false in
set_option linter.unusedSimpArgs false in
set_option maxHeartbeats 800000 in -- 16 cases, up to 9 linear_combination attempts each
theorem qubit_t_m4_decomposition (x : Basis4) :
    rhsAmplitude x = targetAmplitude x := by
  have hI : Complex.I * Complex.I = -1 := Complex.I_mul_I
  have h2 : (Real.sqrt 2 : ℂ) * (Real.sqrt 2 : ℂ) = 2 := by exact_mod_cast sqrt2_sq
  have hI2 : Complex.I ^ 2 = -1 := Complex.I_sq
  have hI3 : Complex.I ^ 3 = -Complex.I := by
    rw [pow_succ, hI2]; ring
  generalize hx0 : x 0 = x0
  generalize hx1 : x 1 = x1
  generalize hx2 : x 2 = x2
  generalize hx3 : x 3 = x3
  fin_cases x0 <;> fin_cases x1 <;> fin_cases x2 <;> fin_cases x3
  all_goals
    simp (config := { decide := true }) only
      [rhsAmplitude, targetAmplitude, hammingWeight, bval,
       sigma1Amp, sigma2Amp, sigma3Amp, inv_two_sqrt2, c1_eq, c2, c3_eq,
       hx0, hx1, hx2, hx3, Fin.val_zero, Fin.val_one, Fin.isValue,
       pow_zero, pow_one, hI2, hI3, mul_one, one_mul, mul_zero, zero_mul,
       add_zero, zero_add, if_true, if_false, Nat.reduceAdd, Nat.reduceMul,
       Nat.reducePow]
    push_cast
  all_goals first
    | ring1
    | linear_combination ((Real.sqrt 3 : ℂ) + 1) / 12 * h2
    | linear_combination
        (Complex.I * (Real.sqrt 3 : ℂ) - Complex.I + (Real.sqrt 3 : ℂ) + 1) / 24 * h2
    | linear_combination (-((Real.sqrt 2 : ℂ) ^ 2 * ((Real.sqrt 3 : ℂ) - 1)) / 24) * hI
        + (Complex.I * (Real.sqrt 3 : ℂ) + Complex.I + (Real.sqrt 3 : ℂ) - 1) / 24 * h2
    | linear_combination (-((Real.sqrt 2 : ℂ) ^ 2 * ((Real.sqrt 3 : ℂ) - 1)) / 24) * hI
        + (Complex.I - 1) / 12 * h2
    | linear_combination
    ...
    | linear_combination ((Real.sqrt 2 : ℂ) ^ 2 * ((Real.sqrt 3 : ℂ) - 1) / 24) * hI
        + (Complex.I * (Real.sqrt 3 : ℂ) + Complex.I - (Real.sqrt 3 : ℂ) + 1) / 24 * h2
    | linear_combination (-((Real.sqrt 2 : ℂ) ^ 2 * ((Real.sqrt 3 : ℂ) - 1)) / 12) * hI
        + ((Real.sqrt 3 : ℂ) - 1) / 12 * h2
