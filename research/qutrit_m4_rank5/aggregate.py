"""Certificate-side check of the batch outputs of the rank-5 exclusion of
|M>^4 (M = N or H3).

    aggregate.py ORBIT [--partition P] [--results-dir D] [--dry-run] [--partial]
...
names and (unless --no-reenumerate) equals a fresh enumeration of the
dependent and repeated full 5-covers; that the stage B and C cover ids tile
the list exactly once, with every stage B cover of distinct states and every
stage C cover repeated; every batch file present; each file's two hashes
(deterministic part, whole record); geometry, partition hash, degenerate
hash, cell and base point equal to the partition's; every `undecided` list
empty; every hit re-decided here from its phase codes (|M>^4 against the
...
total equal to the census's. Then, unless --dry-run, it re-runs N batches
from scratch (indices drawn from numpy's default_rng(S) over the batches
present, S printed on a `seed:` line) into a scratch directory and compares
the deterministic hash of each with the stored one, bit for bit. When the
stored checks pass it writes batch_manifest_ORBIT.json (one entry per batch
with its parameters, output path and SHA-256, the manifest
`certificate.attested.batches` names), or batch_manifest_ORBIT.partial.json
while batches are missing.

Prints `CERTIFIED chi(ORBIT^4) >= 6` only when every batch is present, every
...
from common import M, N1, RANK, X0  # noqa: E402
...
def claim(orbit):
    return f"CERTIFIED chi({orbit}^4) >= 6"
...
def check_batch(orbit, rec, part, geo, problems):
    ...
    for key, want in (("orbit", orbit), ("m", M), ("rank", RANK), ("n1", N1), ("N", part["N"]),
                      ("group_order", part["group_order"]), ("x0", list(X0[orbit]))):
        if rec.get(key) != want:
            problems.append(f"batch {idx}: {key} = {rec.get(key)}, expected {want}")
    if rec.get("matched", 0) + rec.get("refused", 0) + len(rec.get("undecided", [])) != rec.get("covers", 0):
        problems.append(f"batch {idx}: matched + refused + undecided runs do not equal the covers")
...
def check_stage_a_units(orbit, part, problems):
    from cover_census import CoverEnumerator3
    E = CoverEnumerator3(orbit, common.N2)
    ...
    if list(X0[orbit]) != part["x0"]:
        problems.append("the partition's base point is not the cell's")
    return E


def check_degenerate(orbit, part, reenumerate, problems, E=None):
    ...
    try:
        covers, _ = common.load_degenerate(orbit, path, part["degenerate"]["sha256"])
    ...
    if reenumerate:
        from driver import degenerate_covers, small_covers
        from cover_census import CoverEnumerator3
        t0 = time.time()
        E = E or CoverEnumerator3(orbit, common.N2)
        covers3, covers4 = small_covers(orbit, E)
        fresh = degenerate_covers(E, RANK, covers3, covers4)
        same = [tuple(c) for c in fresh] == covers
        if not same:
        ...
        print(f"degenerate covers re-enumerated: {len(fresh)} in {time.time() - t0:.0f}s, "
              f"{'equal to' if same else 'DIFFERENT from'} the stored list")
    return covers


def write_manifest(orbit, path, part, records, results_dir, complete):
    ...
    doc = {"orbit": orbit, "partition": os.path.relpath(common.partition_path(orbit), common.ROOT),
...
def rerun(orbit, part_path, index, out_dir):
    cmd = ["nice", "-n", "19", sys.executable, os.path.join(common.HERE, "batch.py"), orbit, str(index),
...
def main(argv):
    ...
    ap.add_argument("--manifest", default=None,
                    help="where to write the batch manifest (default batch_manifest_ORBIT.json, or "
                         "batch_manifest_ORBIT.partial.json when batches are missing)")
    ...
    ap.add_argument("--recheck-seed", type=int, default=20260922)
    ap.add_argument("--recheck-dir", default=None)
    a = ap.parse_args(argv[1:])
    common.lower_priority()
    orbit = a.orbit
    part_path = a.partition or common.partition_path(orbit)
    results_dir = a.results_dir or common.results_dir(orbit)
    t_all = time.time()
    problems = []
    try:
        part = common.load_partition(orbit, part_path)
    except (OSError, ValueError) as exc:
        print(f"NOT CERTIFIED: {exc}")
        return 1
    B = part["batches"]
    print(f"partition {os.path.relpath(part_path, common.ROOT)}: {orbit} m={M} rank={RANK} x0={tuple(part['x0'])}, "
          f"{B} batches (A {part['stage_a_batches']}, B {part['stage_b_batches']}, "
          f"C {part['stage_c_batches']}), sha256 {part['sha256'][:16]}")
    E = check_stage_a_units(orbit, part, problems)
    check_degenerate(orbit, part, not a.no_reenumerate, problems, E)
    records, missing = {}, []
    for geo in part["batch_geometry"]:
        idx = geo["index"]
        path = os.path.join(results_dir, f"batch_{idx}.json")
        if not os.path.exists(path):
            missing.append(idx)
            continue
        with open(path) as f:
            records[idx] = json.load(f)
        check_batch(orbit, records[idx], part, geo, problems)
    present = set(records)
    ...
    if all(i in present for i in by_stage["A"]):
        ...
        if census_covers is not None and tot["A"]["covers"] != census_covers:
            problems.append(f"stage A matched {tot['A']['covers']} covers, the census counted "
                            f"{census_covers}")
    found = redecide_hits(orbit, records, problems)
    ...
    if found:
        idx, h = found[0]
        print(f"DECOMPOSITION FOUND: |{orbit}>^4 lies in the span of {h['terms_count']} stabilizer states "
              f"(batch {idx}, numerical rank {h['rank']}, base cover {h['cover']} at x0 {tuple(h['x0'])}, "
              f"residual {h['residual']:.2e}); chi({orbit}^4) <= {h['rank']}; {len(found)} such hit(s)")
        return 2
    ...
    if missing:
        ...
        print("every stored check passes: all batches present, hashes verify, geometry matches the "
              ...
              f"list is tiled exactly once, no undecided run, no hit is a decomposition of |{orbit}>^4")
    if records and not a.no_manifest:
        manifest = a.manifest or common.manifest_path(orbit, complete=not missing)
        write_manifest(orbit, manifest, part, records, results_dir, complete=not missing)
    if a.dry_run:
    ...
    out_dir = a.recheck_dir or tempfile.mkdtemp(prefix=f"{orbit}_m4_rank5_recheck_")
    print(f"seed: {a.recheck_seed}")
    print(f"re-running {n} batch(es) from scratch: {picks} -> {out_dir}")
    mismatches = []
    for idx in picks:
        proc, dt = rerun(orbit, part_path, idx, out_dir)
