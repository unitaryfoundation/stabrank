"""One batch of the rank-5 exclusion of |T>^5 by a two-qubit base slice at
the base point 00.
...
5-cover enumeration of |T>^3: the compiled kernel lists the full 5-covers
of the pair and the stage (alpha) matcher runs each at 00), a stage B
batch (indices into degenerate5.json: dependent 5-covers of five distinct
states, through the coefficient family), a stage C batch (covers with a
repeated state, through the block treatment), a stage beta batch (indices
into the rank-4 record's covers4.json: full 4-covers of |T>^3, each run
with the invisible term on every one of the six flats missing 00, through
invisible.InvisibleMatcher.run_one), or a stage gamma batch (indices into
the full 3-covers, each run with every one of the 21 flat pairs, through
InvisibleMatcher.run_two). See docs/notes/t5_rank5_exclusion.md.
...
psi_5 in the span is a decomposition with at most five terms, and the
batch exits 2 with DECOMPOSITION FOUND on its last stdout line. A run that
raises (UnpinnedFamily, the candidate cap, anything else), a hit on which
the two decisions disagree, and every run not made before --max-seconds
elapsed are recorded under `undecided`, which fails the batch (exit 1) and
the aggregate. Exit 0 when every run was made and no hit is a
decomposition.
...
bit; the count of runs through the compiled kernel, the modular candidate
count, timing, host, matcher and version fields follow outside it, then
`sha256` over the whole record.
...
from common import FLAT_NAMES, FLAT_PAIRS, M, N1, ORBIT, RANK, RUNS_PER_COVER, X0, X0S  # noqa: E402
from invisible import InvisibleMatcher  # noqa: E402
from slice_cover import SliceMatcher, _reduce  # noqa: E402
...
def run_units(stage):
    """The runs of one cover: the base point for the stage (alpha) kinds,
    the six flats for stage beta, the 21 flat pairs for stage gamma."""
    if stage == "beta":
...
def match_cover(matcher, cover, rec, numerics, extra, stage, deadline=None):
    """One cover through every one of its runs, accumulated into the record."""
    for label, unit in run_units(stage):
        if deadline is not None and time.time() > deadline:
            rec["undecided"].append({"cover": [int(u) for u in cover], "unit": label, "reason": "deadline: not run"})
            continue
        try:
            if stage == "beta":
                hits, st = matcher.run_one(cover, unit)
            elif stage == "gamma":
                hits, st = matcher.run_two(cover, unit)
            else:
                hits, st = matcher.run(cover, unit)
        except Exception as exc:                      # noqa: BLE001
            rec["undecided"].append({"cover": [int(u) for u in cover], "unit": label,
        ...
        extra["candidates"] += int(st.get("candidates", 0))   # a modular superset count, outside as well
        k = hist_key(stage, st)
        rec["coord_solution_hist"][k] = rec["coord_solution_hist"].get(k, 0) + 1
        flats = None if stage in ("A", "B", "C") else ([unit] if stage == "beta" else list(unit))
        for h in hits:
            det, num = common.hit_record(h, cover, X0, flats)
            rec["hits"].append(det)
            numerics.append(num)
            if not det["agree"]:
...
def run_batch(part, index, native, max_seconds=None, verbose=False):
    ...
    if stage in ("beta", "gamma"):
        matcher = InvisibleMatcher(E, verbose=verbose)
    else:
    ...
    rec = {"batch": {"index": index, "stage": stage},
           ...
           "partition_sha256": part["sha256"], "degenerate_sha256": None, "covers4_sha256": None,
           "covers": 0, "matched": 0, "refused": 0, "coord_solution_hist": {},
           "hits": [], "hit_count": 0, "decompositions": 0, "undecided": []}
    numerics = []
    extra = {"candidates": 0, "native_runs": 0, "kernel_s": 0.0, "match_s": 0.0, "deadline_hit": False}
    t0 = time.time()
    ...
    if stage == "A":
        ...
        for i, j, _ in units:
            ...
            for cover in sorted(covers):
                match_cover(matcher, cover, rec, numerics, extra, stage, deadline)
            extra["match_s"] += time.time() - tm
    else:
        if stage == "beta":
            path = os.path.join(common.HERE, part["covers4"]["file"])
            all_covers, doc = common.load_covers4(path, part["covers4"]["sha256"])
            rec["covers4_sha256"] = doc["sha256"]
        elif stage == "gamma":
            all_covers = [tuple(int(x) for x in T) for T in part["covers3"]["covers"]]
        else:
    ...
    extra["matcher"] = ("invisible" if stage in ("beta", "gamma") else
...
def main(argv):
    ...
    ap.add_argument("--max-seconds", type=float, default=None,
                    help="stop matching after S seconds; runs not made are recorded as undecided")
    ...
    print(f"batch {index} of {part['batches']}: stage {geo['stage']}, {size}, "
          f"{RUNS_PER_COVER[geo['stage']]} run(s) per cover, {'compiled kernels' if native else 'Python reference'}"
    ...
    ru = os.times()
    rec.update({
        "native_runs": extra["native_runs"], "candidates": extra["candidates"],
    ...
    if rec["decompositions"]:
        ...
        print(f"DECOMPOSITION FOUND batch {index}: psi_5 lies in the span of {h['terms_count']} "
              ...
              f"x0 {h['x0']}" + (f" with invisible flats {h['flats']}" if "flats" in h else "")
              + f"; chi(qubit_T^5) <= {h['rank']}; {rec['decompositions']} such hit(s)", flush=True)
        return 2
    if rec["undecided"]:
        n_dead = sum(str(u.get("reason", "")).startswith("deadline") for u in rec["undecided"])
        print(f"batch {index}: {len(rec['undecided'])} undecided run(s), {n_dead} of them not made before the "
