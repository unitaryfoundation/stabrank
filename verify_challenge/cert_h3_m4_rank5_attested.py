"""Certificate: chi(|H3>^{ot 4}) >= 6, by the exclusion of every rank-5
...
one-qutrit slice of a rank-5 decomposition of psi_4 = |H3>^4 has all five
terms nonzero (Fact A: chi(H3^3) = 4 and the unique rank-4 decomposition of
|H3>^3 does not lift, research/constructions/relaxed_lift.py case [A]), and
...
symmetry of |H3> on qutrits 1, 2 carries the decomposition to one whose
slice at x_0 = (1, 1) has all five terms nonzero. That slice is a full
5-cover of |H3>^2 by two-qutrit stabilizer states, and the two-qutrit slice
structure lemma fixes the eight other slices of every term up to a finite
set of Pauli classes and cube-root phases. research/qutrit_m4_rank5
enumerates every full 5-cover of |H3>^2 up to the unitary symmetry of
|H3>^2 (stage A: distinct independent states; stage B: dependent; stage C:
a repeated state), matches each at x_0 modulo 65521 (a superset of the true
matches) and re-decides every candidate mod 2013265921 and in floating
point. No match is a decomposition of psi_4, so chi(H3^4) >= 6; with the
rank-8 witness bounds/H3-m4-upper-8.json the cell stands at
6 <= chi(H3^4) <= 8.

What this script re-runs: research/qutrit_m4_rank5/aggregate.py H3 checks
...
(research/qutrit_m4_rank5/results/H3/control_*.json) are part of the record
but are not re-run here.

Printed claims: CERTIFIED chi(H3^4) >= 6
"""
