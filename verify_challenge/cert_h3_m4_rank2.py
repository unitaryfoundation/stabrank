"""Certificate: chi(|H3>^{ot 4}) >= 3, by excluding every rank-2 decomposition
...
(cert_h3_m4_rank3.py) is the stronger certificate and takes longer than
the default budget.

Printed claim: CERTIFIED chi(H3^4) >= 3
"""
...
def main():
    ...
    pairs, worst = rank2_search_codes(_Geometry(psi_for("H3", 4), codes, ks))
    if pairs == "RANK1" or pairs:
        print(f"H3 m=4: rank <= 2 decomposition FOUND {pairs if pairs != 'RANK1' else ''}")
        return 1
    slack = (1 - PARALLEL) - worst
    print(f"H3 m=4: no pair spans it; closest approach {worst:.6f}, clearing by {slack:.4f}")
    if slack < MARGIN:
        print("margin too thin to stand behind", file=sys.stderr)
        return 1
    print("CERTIFIED chi(H3^4) >= 3")
    return 0
