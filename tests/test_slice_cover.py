...
@pytest.fixture(scope="module")
def enumT2():
    return CoverEnumerator(2, orbit="qubit_T")


def test_t_field_constants_and_the_unchanged_h_field():
    """The qubit_T field shares i (hence every phase code) with the qubit_H
    field and the compiled kernels, its ratio tau satisfies tau^2 = i (2 -
    sqrt 3) (tan(beta)^2 = 2 - sqrt 3), and the H field's ratio is tan(pi/8)
    as before; both primes are 1 mod 24."""
    from slice_cover import ratio_complex
    for p in (P1, P2):
        assert p % 24 == 1
        FH, FT = Field(p), Field(p, "qubit_T")
        assert FT.i == FH.i and FT.zeta == FH.zeta
        assert FH.ratio == FH.tan
        assert pow(FT.sqrt2, 2, p) == 2 and pow(FT.sqrt3, 2, p) == 3
        assert (pow(FT.ratio, 2, p) - FT.i * (2 - FT.sqrt3)) % p == 0
        assert list(FH.target(2)) == [FH.cos * FH.cos % p, FH.cos * FH.sin % p, FH.cos * FH.sin % p,
                                      FH.sin * FH.sin % p]
        assert list(FT.target(2)) == [1, FT.ratio, FT.ratio, FT.ratio * FT.ratio % p]
    tau = ratio_complex("qubit_T")
    assert abs(tau ** 2 - 1j * (2 - np.sqrt(3))) < 1e-12
    from rank_exclusion import psi_for
    a = psi_for("qubit_T", 1)
    assert abs(a[1] / a[0] - tau) < 1e-12
    with pytest.raises(ValueError):
        Field(P1, "qubit_S")


def _recover_stored(E, m, n1, rank, orbit, native):
    """Every stored rank-r decomposition of |orbit>^m with an all-visible
    slice along the first n1 qubits at x0 in (0, 1) is among the matcher's
    hits from that base; returns the number of (decomposition, x0) runs."""
    from common import load_decompositions
    lookup = {E.codes[i].tobytes(): i for i in range(E.N)}
    M = SliceMatcher(E, n1, native=native)
    decs, _ = load_decompositions(orbit, m, rank)
    tried = 0
    for terms, _ in decs:
        for x0 in (0, 1):
            slices = [t.reshape(1 << n1, -1)[x0] for t in terms]
            if any(np.linalg.norm(s) < 1e-9 for s in slices):
                continue
            cover = tuple(lookup[exact_codes(s)[0].tobytes()] for s in slices)
            hits, st = M.run(cover, x0)
            assert not st["refused"], (cover, x0, st)
            if native:
                assert st["native"] == (len(set(cover)) == len(cover) and st["kappa"] == 0), st
            want = _codes_key(terms)
            assert any(_codes_key(h["terms"]) == want and h["rank"] == rank and h["exact"] and h["nonzero"]
                       for h in hits), (cover, x0, st)
            tried += 1
    return tried


def test_t_matcher_recovers_the_rank3_decompositions_of_t3(enumT2):
    """The 8 stored rank-3 decompositions of |T>^3 from their one-qubit
    slices (3-covers of |T>^2 over the 60 two-qubit states)."""
    assert _recover_stored(enumT2, 3, 1, 3, "qubit_T", native=False) >= 8


def test_t_matcher_recovers_the_rank3_decomposition_of_t4(enumT3):
    """The unique rank-3 decomposition of |T>^4 (the base of the rank-3
    exclusion at m = 5) from both of its one-qubit slices, through the
    compiled kernel and through the Python reference."""
    assert _recover_stored(enumT3, 4, 1, 3, "qubit_T", native=False) == 2
    if _native_available():
        assert _recover_stored(enumT3, 4, 1, 3, "qubit_T", native=True) == 2


def test_t_covers_match_the_stored_lists(enumT2, enumT3):
    """The enumerator's full 2-covers of |T>^2 and full 3-covers of |T>^3
    are the stored minimal decompositions up to the unitary symmetry of the
    target: the same orbits, canonicalised under the group generated by the
    enumerator's permutations."""
    from common import load_decompositions

    def group(E):
        gens = [np.asarray(p, dtype=np.int32) for p in E.info["perms"]]
        ident = np.arange(E.N, dtype=np.int32)
        seen, frontier = {ident.tobytes(): ident}, [ident]
        while frontier:
            nxt = []
            for g in frontier:
                for h in gens:
                    k = h[g]
                    if k.tobytes() not in seen:
                        seen[k.tobytes()] = k
                        nxt.append(k)
            frontier = nxt
        assert len(seen) == E.info["order"]
        return list(seen.values())

    def canon(idx, G):
        return min(tuple(sorted(int(x) for x in g[list(idx)])) for g in G)

    for E, m, r in ((enumT2, 2, 2), (enumT3, 3, 3)):
        G = group(E)
        lookup = {E.codes[i].tobytes(): i for i in range(E.N)}
        covers, _ = E.covers(r)
        got = {canon(c, G) for c in covers}
        # the pivot and partner reductions leave at most a few duplicates per orbit
        assert len(covers) <= 2 * len(got)
        if m == 2:
            assert len(got) == 1
            continue
        decs, _ = load_decompositions("qubit_T", m, r)
        stored = {canon([lookup[exact_codes(t)[0].tobytes()] for t in terms], G) for terms, _ in decs}
        assert got == stored, (len(got), len(stored))


def test_t_native_kernel_matches_reference_on_t3_bases(enumT3):
    """On a sample of the full 4-covers of |T>^3 at n1 = 2 (the bases of
    the rank-4 exclusion at m = 5) the compiled kernel and the reference
    agree run by run: same coordinate-slice solution counts, same hits."""
    if not _native_available():
        pytest.skip("stabrank_core without SliceMatchKernel")
    E = enumT3
    covers, _ = E.covers(4, max_found=40, pivots=E.reps[:2])
    assert len(covers) >= 20
    ref, nat = SliceMatcher(E, 2, native=False), SliceMatcher(E, 2)
    key = lambda hits: sorted(tuple(sorted(exact_codes(t)[0].tobytes() for t in h["terms"])) for h in hits)
    runs = 0
    for cover in covers[:40]:
        for x0 in (0, 1):
            h_ref, s_ref = ref.run(cover, x0)
            h_nat, s_nat = nat.run(cover, x0)
            assert s_nat["native"] and not s_ref["native"]
            assert s_nat["coord_solutions"] == s_ref["coord_solutions"], (cover, x0)
            assert key(h_nat) == key(h_ref), (cover, x0)
            runs += 1
    assert runs >= 40
...
def test_h_enumerator_unchanged_by_the_orbit_parameter(enum3):
    """The default enumerator is the qubit_H one of before: the same modular
    targets, ratio and symmetry group as an explicit orbit="qubit_H"."""
    E = enum3
    assert E.orbit == "qubit_H" and E.F1.orbit == "qubit_H"
    assert E.ratio == np.tan(np.pi / 8)
    assert list(E.psi1) == list(Field(P1).target(3)) and list(E.psi2) == list(Field(P2).target(3))
    assert E.info["order"] == 48 and E.N == 1080
    EH = CoverEnumerator(3, orbit="qubit_H")
    assert np.array_equal(EH.psi1, E.psi1) and np.array_equal(EH.reps, E.reps)
